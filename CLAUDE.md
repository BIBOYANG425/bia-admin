...
Visit http://localhost:3000. Sign in with Google OAuth. Your `auth.users` row must have a matching row in `admin_users` (with role `super_admin`, `editor`, or `viewer`) or `AdminShell` redirects to `/login?denied=not-invited` (middleware only refreshes the session cookie).
...
- **Pickup QR** → the admin pickup desk (`/admin/shipping/pickup`) has a camera scanner that decodes the **bare 8-char `pickup_token`** and feeds the existing verify endpoint. bia-roommate must render each parcel's `pickup_token` as a QR (raw token text — no URL/JSON wrapper) on the student pickup view — **still outstanding as of 2026-07-03** (the student page shows the token as text only; ~10 lines with the `qrcode` package). The admin decoder is lenient (also tolerates `?t=<token>` / last-path-segment URLs or `{token}` JSON), but raw token is the agreed contract.
- **Student self-confirm pickup gated (D1, 2026-07-03)** → `student_confirm_pickup` now RAISEs `pickup_not_open` (parent shipment not `pickup_open`) and `payment_required` (recorded `amount_owed_cents` unpaid) in addition to the existing tokens (migration `20260703000003`). bia-roommate's confirm-pickup route/UI should map these to friendly copy（取件窗口未开放 / 请先完成付款）and ideally hide the 确认取件 button when the window isn't open. Officer desk paths are unaffected.
