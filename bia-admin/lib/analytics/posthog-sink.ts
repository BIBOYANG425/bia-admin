// PostHog-backed AnalyticsSink for bia-admin. Returns null (→ the shared
// analytics façade stays a no-op) when NEXT_PUBLIC_POSTHOG_KEY is unset, so the
// app is safe to ship before a PostHog project/key exists. Explicit events only
// (autocapture off) so we never leak PII out of the DOM.
